...
Shader "ClipStudioLike/UI/14_ソフトライト"
{
    Properties
    {
...
# endif
                    ...
                    {
                        color.rgb = 2 * srcColor * color.rgb + (srcColor * srcColor) * (1 - 2 * color.rgb);
                    }
                    else
                    {
                        #pragma warning 微妙に違う
                        color.rgb = 2 * srcColor * (1 - color.rgb) + sqrt(srcColor) * (2 * color.rgb - 1);
                    }
